...
test('a change always opens on its newest version', async ({ page }) => {
  await page.locator('#read-set').selectOption('1');
  await expect(page.locator('#read-set')).toHaveValue('1');

  // Another change, and back. Reading an older version of one change says
  // nothing about which version of the next one the reader wants, and the
  // numbers of two changes are not the same versions.
