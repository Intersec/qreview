- qreview is much faster on a large repository. The file list of a change is
  ...
  only as far as its hunks reach, and the tags are read once per walk
  instead of once per commit. A file that took 2.7 s to open now takes
  0.6 s, and 0.03 s the second time.
