...
mod tests {
    ...
    async fn the_auto_merge_shows_the_resolution_and_nothing_else() {
        ...
        let files = diff::files(&git, &tree, &head.hash, &diff::How::default())
            .await
            .unwrap();
        ...
        let file = diff::file(&git, &tree, &head.hash, "f", None, &diff::How::default())
            .await
            .unwrap()
            .unwrap();
    ...
    async fn the_first_parent_shows_everything_the_merge_brought() {
        ...
        let files = diff::files(&git, &base, &head.hash, &diff::How::default())
            .await
            .unwrap();
